...
namespace SWLOR.Game.Server.Perk.ForceAlter
{
    public class Confusion : IPerkHandler
    {
        ...
        {
            float radiusSize = NWScript.RADIUS_SIZE_SMALL;            
            ...
            {
                case 1:
                    if ((creature.Wisdom > NWScript.GetAbilityModifier(NWScript.ABILITY_WISDOM, target) || creature == target) && NWScript.GetDistanceBetween(creature.Object, target) <= radiusSize)
                    {
                        creature.AssignCommand(() =>
                        {
                            NWScript.ApplyEffectToObject(DurationType.Temporary, confusionEffect, target, 6.1f);
                            // Play VFX
                            NWScript.ApplyEffectToObject(DurationType.Instant, NWScript.EffectVisualEffect(NWScript.VFX_IMP_CONFUSION_S), target);
                        });
                    ...
                    break;
                case 2:
                    NWCreature targetCreature = NWScript.GetFirstObjectInShape(NWScript.SHAPE_SPHERE, radiusSize, creature.Location, 1, ObjectType.Creature);
                    while (targetCreature.IsValid)
                    {
                        if (targetCreature.RacialType == (int)CustomRaceType.Robot || NWScript.GetIsReactionTypeHostile(targetCreature, creature) == 0)
                        {
                            // Do nothing against droids or non-hostile creatures, skip object
                            targetCreature = NWScript.GetNextObjectInShape(NWScript.SHAPE_SPHERE, radiusSize, creature.Location, 1, ObjectType.Creature);
                            continue;
                        }
                        ...
                        if (creature.Wisdom > targetCreature.Wisdom)
                        {
                            ...
                            creature.AssignCommand(() =>
                            {
                                ...
                                // Play VFX
                                NWScript.ApplyEffectToObject(DurationType.Instant, NWScript.EffectVisualEffect(NWScript.VFX_IMP_CONFUSION_S), targetCreatureCopy);
                            });
