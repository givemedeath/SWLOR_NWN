...
namespace SWLOR.Game.Server.Service
{
    public static class SpaceService
    {
        ...
        public static string GetPlanetFromLocation(string location)
        {
            ...
            if (hyphen == -1)
            {
                ...
                if (dock != null)
                {
                    ...
                    {
                        if (NWScript.GetResRef(area) == parentBase.AreaResref)
                        {
                            location = NWScript.GetName(area);
        ...
        public static NWPlaceable GetCargoBay(NWArea starship, NWPlayer player)
        {
            ...
            NWLocation location = (player != null ? player.Location : (NWLocation) NWScript.Location(starship, NWScript.Vector(1, 1, 0), 0));
        ...
        {
            ShipStats stats = GetShipStatsByAppearance(NWScript.GetAppearanceType(ship));
            ...
            ship.SetLocalInt("WEAPONS", stats.weapons + GetCargoBonus(bay, (int)ItemPropertyType.StarshipWeaponsBonus));
            ship.SetLocalInt("SHIELDS", stats.shields + GetCargoBonus(bay, (int)ItemPropertyType.StarshipShieldsBonus));
            ship.SetLocalInt("STEALTH", stats.stealth + GetCargoBonus(bay, (int)ItemPropertyType.StarshipStealthBonus));
            ship.SetLocalInt("SCANNING", stats.scanning + GetCargoBonus(bay, (int)ItemPropertyType.StarshipScanningBonus));
            ship.SetLocalInt("SPEED", stats.speed + 25 * GetCargoBonus(bay, (int)ItemPropertyType.StarshipSpeedBonus));
            ship.SetLocalInt("STRONIDIUM", pcBase.ReinforcedFuel);
            ship.SetLocalInt("HP", (int) structure.Durability);
            ship.SetLocalFloat("RANGE", stats.range + GetCargoBonus(bay, (int)ItemPropertyType.StarshipRangeBonus));
        }
        ...
        public static int GetCargoBonus(NWPlaceable bay, int stat)
        {
            ...
            foreach (var item in bay.InventoryItems)
            {
                ...
                ItemProperty prop = NWScript.GetFirstItemProperty(item);

                while (NWScript.GetIsItemPropertyValid(prop) == 1)
                {
                    if (NWScript.GetItemPropertyType(prop) == stat) bonus += NWScript.GetItemPropertyCostTableValue(prop);
                    prop = NWScript.GetNextItemProperty(item);
                }
            }
        ...
        public static Hashtable GetLandingDestinationList(NWPlayer player, PCBase pcBase)
        {
            ...
            foreach (var area in NWModule.Get().Areas)
            {
                ...
                if (NWScript.GetName(area).StartsWith(planet))
                {
                    LoggingService.Trace(TraceComponent.Space, "Checking area " + NWScript.GetName(area) + " for landing spots.");
                                    ...
                                    foreach (var plc in areaStructures)
                                    {
                                        if (plc.PCBaseStructureID == structure.ID)
                                        {
                                            ...
                                            if (plc.Structure.GetLocalInt("DOCKED_STARSHIP") == 0)
                                            {
                                                ...
                                                landingSpots.Add("Dock " + Count.ToString() + " in " + NWScript.GetName(area) + " (" + PlayerService.GetPlayerEntity(@base.PlayerID).CharacterName + ")", structure.ID);
                                                Count++;
                                            }
                                        }
                                    }
        ...
        public static bool DoPilotingSkillCheck(NWPlayer player, int DC, bool reRollIfFailed = false)
        {
            ...
            if (NWScript.d100() <= (100 * rank) / (rank + DC) ||
                (reRollIfFailed && NWScript.d100() <= (100 * rank) / (rank + DC)))
            {
                // Success!
        ...
        public static void CreateShipInSpace(NWArea ship, NWLocation location = null)
        {
            ...
            if (location == null)
            {
                ...
                NWObject waypoint = NWScript.GetObjectByTag(Regex.Replace(planet, @"\s+", "") + "_Orbit");
            ...
            NWCreature shipCreature = NWScript.CreateObject(ObjectType.Creature, "starship" + shipBase.BuildingStyleID.ToString(), location, 0, shipID);
            ...
            NWScript.AssignCommand(shipCreature, () => { UpdateCargoBonus(ship, shipCreature); });
        }
        ...
        public static bool CanLandOnPlanet(NWArea ship)
        {
            ...
            if (!shipCreature.IsValid || !waypoint.IsValid || NWScript.GetDistanceBetween(shipCreature, waypoint) < 10.0f)
            {
                return true;
            }
        ...
        public static void DoFlyShip(NWPlayer player, NWArea ship)
        {
            ...
            int shipSpeed = GetShipStatsByAppearance(shipAppearance).speed + 
                            25 * GetCargoBonus(GetCargoBay(ship, null), (int)ItemPropertyType.StarshipSpeedBonus);

            NWPlaceable chair = NWScript.GetNearestObjectByTag("pilot_chair", player);
            ...
            player.Chest.SetLocalInt("APPEARANCE", NWScript.GetAppearanceType(player));
            player.SetLocalInt("IS_SHIP", 1);
            player.SetLocalObject("AREA", ship);
            NWScript.SetCreatureAppearanceType(player, shipAppearance);

            NWScript.SetObjectVisualTransform(player, OBJECT_VISUAL_TRANSFORM_SCALE, GetShipStatsByAppearance(shipAppearance).scale);

            // Make immune to physical damage.
            NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, NWScript.EffectDamageImmunityIncrease(DAMAGE_TYPE_BLUDGEONING, 100), player);

            player.AssignCommand(() => { NWScript.ActionUnequipItem(player.LeftHand); });
            player.AssignCommand(() => { NWScript.ActionUnequipItem(player.RightHand); });
            ...
            if (shipSpeed > 100)
            {
                NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, NWScript.EffectMovementSpeedIncrease(shipSpeed - 100), player);
            }
            else
            {
                NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, NWScript.EffectMovementSpeedDecrease(100 - shipSpeed), player);
            }
            
            // Clean up the ship model.
            player.AssignCommand(() => { NWScript.ActionJumpToLocation(shipCreature.Location); });
        ...
        public static void DoStopFlyShip(NWPlayer player)
        {
            ...
            Effect eGhost = NWScript.EffectCutsceneGhost();
            NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eGhost, player, 3.5f);
            NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eGhost, copy, 3.5f);
            copy.Area.DeleteLocalObject("CREATURE");
            CreateShipInSpace(copy.Area, player.Location);
            player.AssignCommand(() => { NWScript.ClearAllActions(); NWScript.ActionJumpToLocation(copy.Location); });

            // Make ourselves invisible for 2.5s and destroy the copy at the same time.  
            NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, NWScript.EffectVisualEffect(VFX_DUR_CUTSCENE_INVISIBILITY), player, 2.5f);
            copy.Destroy(2.5f);

            // Set our appearance back to normal (now that we're invisible). 
            NWScript.SetCreatureAppearanceType(player, player.Chest.GetLocalInt("APPEARANCE"));
        ...
        public static void DoCrewGuns(NWPlayer player, NWArea ship)
        {
            ...
            NWPlaceable chair = NWScript.GetNearestObjectByTag("gunner_chair", player);
            ClonePCAndSit(player, chair);

            NWCreature shipCreature = ship.GetLocalObject("CREATURE");
            player.Chest.SetLocalInt("APPEARANCE", NWScript.GetAppearanceType(player));
            NWScript.SetCreatureAppearanceType(player, APPEARANCE_TYPE_INVISIBLE_HUMAN_MALE);
            player.SetLocalInt("IS_GUNNER", 1);

            // Apply effects so we can't be seen or hit.
            player.AssignCommand(() => { NWScript.ActionUnequipItem(player.LeftHand); });
            player.AssignCommand(() => { NWScript.ActionUnequipItem(player.RightHand); });
            NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, NWScript.EffectCutsceneGhost(), player);
            NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, NWScript.EffectInvisibility(INVISIBILITY_TYPE_NORMAL), player);
            NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, NWScript.EffectMovementSpeedIncrease(200), player);

            ship.SetLocalObject("GUNNER", player);

            player.AssignCommand(() => 
            {
                NWScript.ActionJumpToLocation(shipCreature.Location);
                NWScript.ActionForceFollowObject(shipCreature);
                NWScript.SetCommandable(0, player);
            });
        ...
        {
            NWScript.SetCommandable(1, player);

            NWCreature copy = player.GetLocalObject("COPY");

            // Apply ghost effect so we can stand on each other's heads.
            NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, NWScript.EffectCutsceneGhost(), copy, 3.5f);
            player.AssignCommand(() => { NWScript.ClearAllActions();  NWScript.ActionJumpToLocation(copy.Location); });
        ...
        private static void ClonePCAndSit(NWPlayer player, NWPlaceable chair)
        {
            ...
            NWObject copy = NWScript.CopyObject(player, player.Location, NWScript.OBJECT_INVALID, "spaceship_copy");
            NWScript.ChangeToStandardFaction(copy, STANDARD_FACTION_DEFENDER);

            Effect eInv = NWScript.EffectVisualEffect(VFX_DUR_CUTSCENE_INVISIBILITY);
            Effect eGhost = NWScript.EffectCutsceneGhost();
            ...
            NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, eGhost, copy);
            NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eInv, player, 2.5f);
            NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eGhost, player, 3.5f);

            // Clear the copy's inventory and gold. (This won't clear equipped items). 
            NWScript.TakeGoldFromCreature(NWScript.GetGold(copy), copy, 1);
            NWItem item = NWScript.GetFirstItemInInventory(copy);
            while (item.IsValid)
            {
                NWScript.SetDroppableFlag(item, FALSE);
                item.Destroy();
                item = NWScript.GetNextItemInInventory(copy);
            }

            // Make sure the clone isn't doing anything silly (like walking waypoints).
            // Then sit in the chair. 
            NWScript.AssignCommand(copy, () => { NWScript.ClearAllActions(); });
            NWScript.DelayCommand(1.0f, ()=> { NWScript.AssignCommand(copy, () => { NWScript.ActionSit(chair); } ); });
        }
        ...
        private static void OnModuleNWNXChat()
        {
            ...
            NWPlayer speaker = NWScript.OBJECT_SELF;
            ...
            if (speaker.GetLocalInt("IS_SHIP") == 1 || speaker.GetLocalInt("IS_GUNNER") == 1)
            {
                ...
                {
                    NWScript.AssignCommand(speaker.GetLocalObject("COPY"), () => { NWScript.SpeakString(message); });
                }
            ...
            int nNth = 1;
            NWCreature copy = NWScript.GetNearestObjectByTag("spaceship_copy", speaker, nNth);

            while (copy.IsValid)
            {
                ((NWPlayer)copy.GetLocalObject("OWNER")).SendMessage(speaker.Name + ": " + message);
                nNth++;
                copy = NWScript.GetNearestObjectByTag("spaceship_copy", speaker, nNth);
            }
        }
        ...
        private static void DoImpactFeedback(NWArea ship, string message)
        {
            foreach (var creature in ship.Objects)
            {
                if (creature.IsPC || creature.IsDM)
                {
                    ...
                    NWScript.ApplyEffectToObject(DURATION_TYPE_INSTANT, NWScript.EffectVisualEffect(VFX_FNF_SCREEN_BUMP), creature);
                    // TODO - play sound.
                }
        ...
        public static void CreateSpaceEncounter (NWObject trigger, NWPlayer player)
        {
            ...
            int random = NWScript.Random(totalChance - 1);
            ...
            foreach (var encounter in encounters)
            {
                if (random < encounter.Chance)
                {
                    ...
                    if (encounter.TypeID == 1 || encounter.TypeID == 4)
                    {
                        ...
                        string resref = NWScript.d2() == 1 ? "pirate_fighter_1" : "pirate_fighter_2";
                        NWCreature pirate = NWScript.CreateObject(OBJECT_TYPE_CREATURE, resref, trigger.Location);
                    ...
                    else if (encounter.TypeID == 2)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Boom!
                                NWScript.ApplyEffectAtLocation(DURATION_TYPE_TEMPORARY, NWScript.EffectVisualEffect(VFX_FNF_IMPLOSION), player.Location, 2.0f);
                                NWScript.ApplyEffectToObject(DURATION_TYPE_INSTANT, NWScript.EffectDeath(), player);
                            }
                            else
                            {
        ...
        private static void DoSpaceAttack(NWCreature attacker, NWCreature target, bool gunner)
        {
            ...
            if (check < (100 * attackerPiloting) / (attackerPiloting + defenderPiloting))
            {
                ...
                Effect eBeam = NWScript.EffectBeam(447, attacker, BODY_NODE_CHEST);
                NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBeam, target, 0.5f);
                ...
                {
                    // Boom!
                    NWScript.ApplyEffectAtLocation(DURATION_TYPE_TEMPORARY, NWScript.EffectVisualEffect(VFX_FNF_IMPLOSION), target.Location, 2.0f);
                    NWScript.ApplyEffectToObject(DURATION_TYPE_INSTANT, NWScript.EffectDeath(), target);
                }
                else
                {
            ...
            else
            {
                ...
                Effect eBeam = NWScript.EffectBeam(447, attacker, BODY_NODE_CHEST, 1);
                NWScript.ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBeam, target, 0.5f);
        ...
        private static void OnCreatureSpawn()
        {
            ...
            ShipStats stats = GetShipStatsByAppearance(NWScript.GetAppearanceType(creature));
            if (stats.scale == default(float)) return;

            NWScript.SetObjectVisualTransform(creature, OBJECT_VISUAL_TRANSFORM_SCALE, stats.scale);
            ...
            // Make immune to physical damage.
            NWScript.ApplyEffectToObject(DURATION_TYPE_PERMANENT, NWScript.EffectDamageImmunityIncrease(DAMAGE_TYPE_BLUDGEONING, 100), creature);
        }
        ...
        private static void ShootValidTarget(NWCreature creature)
        {
            ...
            Location targetLocation = NWScript.Location(
            ...
            NWCreature target = NWScript.GetFirstObjectInShape(shape, range, targetLocation, TRUE, OBJECT_TYPE_CREATURE, creature.Position);
            while (target.IsValid)
            {

                if (NWScript.GetIsEnemy(target, creature) == TRUE &&
                    !target.IsDead &&
                    target.GetLocalInt("IS_GUNNER") == 0 &&
                    NWScript.GetDistanceBetween(creature, target) <= range &&
                ...
                target = NWScript.GetNextObjectInShape(shape, range, targetLocation, TRUE, OBJECT_TYPE_CREATURE, creature.Position);
            }
        }
        ...
        private static void OnCreatureHeartbeat()
        {
            ...
            NWScript.DelayCommand(3.0f, ()=>{ ShootValidTarget(creature); });
        }

        private static void OnModuleEquipItem()
        {
            NWPlayer equipper = NWScript.GetPCItemLastEquippedBy();
            if (equipper.GetLocalInt("IS_CUSTOMIZING_ITEM") == NWScript.TRUE) return; // Don't run heavy code when customizing equipment.
            ...
            if (equipper.GetLocalInt("IS_SHIP") > 0)
            {
                ...
                NWScript.CopyItem(item, equipper, 1);
                item.Destroy();
        ...
        public static void OnPhysicalAttacked(NWCreature creature, NWCreature attacker)
        {
            ...
            if (NWScript.GetDistanceBetween(creature, attacker) < stats.range)
            {
                NWScript.AssignCommand(creature, () => { NWScript.SetFacingPoint(attacker.Position); });
            }
            else
            {
                NWScript.AssignCommand(creature, () => { NWScript.ActionMoveToObject(attacker); });
            }
        }
        ...
        public static void OnPerception(NWCreature creature, NWCreature perceived)
        {
            ...
            if (NWScript.GetIsEnemy(perceived, creature) == 0) return;
            ...
            if (NWScript.GetDistanceBetween(creature, perceived) < stats.range)
            {
                NWScript.AssignCommand(creature, () => { NWScript.ClearAllActions(); NWScript.SetFacingPoint(perceived.Position); });
            ...
            {
                NWScript.AssignCommand(creature, () => { NWScript.ActionMoveToObject(perceived, 1, stats.range - 2); });
            }
        }
        ...
        private static bool AdjustFacingAndAttack(NWCreature creature)
        {
            ...
            NWCreature enemy = NWScript.GetFirstObjectInShape(shape, 25.0f, targetLocation, 1, OBJECT_TYPE_CREATURE);

            while (enemy.IsValid)
            {
                if (NWScript.GetIsEnemy(enemy, creature) == 1)
                {
                    OnPerception(creature, enemy);
                    return true;
                }

                enemy = NWScript.GetNextObjectInShape(shape, 25.0f, targetLocation, 1, OBJECT_TYPE_CREATURE);
            }

            enemy = NWScript.GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, creature);

            if (enemy.IsValid && NWScript.GetDistanceBetween(enemy, creature) < 25.0f)
            {
                ...
                targetLocation = NWScript.Location(
                  creature.Area.Object,
                  BiowarePosition.GetChangedPosition(creature.Position, 5.0f, myFacing),
                  myFacing + 180.0f);

                NWScript.AssignCommand(creature, () => { NWScript.ActionMoveToLocation(targetLocation, 1); });
            }
        ...
        public static void OnHeartbeat(NWCreature creature)
        {
            ...
            if (AdjustFacingAndAttack(creature))
            {
                ...
                NWScript.DelayCommand(3.0f, () => { AdjustFacingAndAttack(creature); });
            }
            else
            {
                // Target not in range, turn!
                NWScript.DelayCommand(2.0f, () => { AdjustFacingAndAttack(creature); });
                NWScript.DelayCommand(4.0f, () => { AdjustFacingAndAttack(creature); });
            }
        }
    }
}
