    ...
    public void FutCityPacked_At32_ComposesCourtyardsMatchingHandBuiltShape()
    {
        ...
        for (var i = 0; i < SeedCount; i++)
        {
            ...
            foreach (var center in centers)
            {
                ...
                ring.Count.Should().BeGreaterThanOrEqualTo(3, "PlanCourtyard commits only with 3+ ring members");
                if (ring.Count >= 4)
                    ring.Select(m => m.Resref).Distinct().Count().Should().BeGreaterThanOrEqualTo(2,
        ...
        courtyardsSeen.Should().BeGreaterThanOrEqualTo(SeedCount,
